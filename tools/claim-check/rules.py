...
_PLACEHOLDER = re.compile(
    ...
    r'|\b(?:N|M|T|S|KEY|MIN|ID|FIELD|PATH|v|i|j|k)\b'
    r'|\\"[^"]*\\"|"[^"]*"'     # any quoted sub-value
    r'|<[^>]+>'                 # <inner CE error>
    r"|'[a-z][a-z-]*'"          # 'key'   — an interpolated key name
    r'|\((?:reason|scope)\)'    # (reason) — an interpolated explanation
    r'|\[\.\.\.\]'             # [...]   — an interpolated slice
    r'|\b(?:positive|non-negative)\b'  # the %s in validateIntOrString
    r'|\.\.\.'
    r'|\d+'
)
...
RULES: list[Rule] = [
    ...
    Rule(
        ...
        # A Rust-internal name, not registered on the Python module. The docs
        # name it while explicitly telling the reader to compare against the
        # literal -1 instead, so the mention is correct and unverifiable by
        # this rule (rust/src/errors.rs:33).
        ignore=("CLIENT_SIDE_RESULT_CODE",),
    ),
    Rule(
        id="acko-conditions",
