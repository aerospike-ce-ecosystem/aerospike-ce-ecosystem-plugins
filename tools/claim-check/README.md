...
placeholders are `N M T S KEY MIN ID FIELD PATH v i j k`, anything in quotes,
`<angle brackets>`, `'key'`, `(reason)`, `(scope)`, `[...]`, `...`, bare digits,
and the words `positive` / `non-negative` (the `%s` in the operator's
`validateIntOrString`). This mirrors the convention
