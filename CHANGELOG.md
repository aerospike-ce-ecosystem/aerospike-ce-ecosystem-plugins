...
- Removed two webhook rules the ACKO webhook does not implement. `acko-operations/reference/validation-rules.md` claimed rack IDs are immutable and that duplicate rack `nodeName`s are rejected; `ValidateUpdate` blocks only a *simultaneous* add-and-remove of rack IDs, and `Rack.NodeName` is never validated (the operator has a test asserting two racks may share a node). Both claims made Claude refuse operations the operator permits.
- Corrected the `metricLabels` constraint: `=` and `,` are safe inside values because the operator TOML-quotes them. The real rules are a key charset of `^[A-Za-z0-9_-]+$` and a control-character ban in values.
- Marked `max-record-size`, `evict-used-pct` and `evict-tenths-pct` as **not** dynamic in ACKO. They are absent from `internal/configdiff/dynamic_params.go`, so editing them cold-restarts the cluster instead of applying via `set-config`. Clarified throughout that the "Dynamic" column tracks ACKO's allowlist, not the server's own capability.
- Corrected `spec.rollingUpdateBatchSize` to integer-only (`*int32`, minimum 1). Percentages such as `"25%"` are valid only on `spec.rackConfig.rollingUpdateBatchSize`, which is `IntOrString` and takes precedence when set.
- Corrected the `security: {}` CRD-mapping row: the config generator skips the section entirely, so it renders nothing into `aerospike.conf`.
- Documented the two TLS surfaces ACKO rejects beyond the top-level `tls` section — nested `network.tls`, and `tls-*` keys inside `network.{service,heartbeat,fabric}`.
- Split the merged namespace-entry error into the two messages the webhook actually produces: `"aerospikeConfig.namespaces[N] must be a map, got T"` and `"aerospikeConfig.namespaces[N] is missing required 'name' key"`. Neither was a substring of the documented `"must be a map with required key 'name'"`.
- Regenerated the drifted webhook error strings from source (storage, monitoring `customRules`, operations, `templateRef`, rack IDs, `validateIntOrString`), restored the U+2013 en dash in the `spec.size must be set (1–8)` message, documented the `validation failed: ` wrapper and `"; "` joining, and noted that an invalid `spec.operations[].kind` is a CRD enum rejection from the API server rather than a webhook error.
- Corrected four `aerospike-py` policy defaults against the pinned `aerospike-core 2.0.0`: `QueryPolicy.max_retries` 2 → 5, `ClientConfig.idle_timeout` 55000 → 30000, `ClientConfig.max_conns_per_node` 300 → 256, and `AdminPolicy.timeout` 1000 → 0 raw / 3000 effective.
