...
- `scripts/verify_skill_claims.py` gains a `webhook-message` category: the backticked, quoted messages in the ACKO error catalogues are now checked against the operator's own format strings. This is the category the first version removed for noise; it is precise here because messages are folded across `+` concatenation, reduced to a skeleton with their `%verbs` marked, and matched **per string** rather than against a global blob of every literal in the repo. Measured at the pinned SHAs: 101 claims checked, 0 reported. Total coverage rises from 321 to 422 claims.
...
- Documented four `aerospike-py` API surfaces the skill omitted entirely: `batch_apply()` (batch UDF; the return value arrives under the Lua `"SUCCESS"` bin key), `PartitionFilter` and its three builder functions for scoping a query to a partition range, `is_connected()` (including the sync/async implementation difference), and the `RustPanicError` exception for legacy blob particle types.

### Fixed

- Removed two webhook rules the ACKO webhook does not implement, and corrected four more (#102, which merged without a changelog entry). `acko-operations/reference/validation-rules.md` claimed rack IDs are immutable and that duplicate rack `nodeName`s are rejected; `ValidateUpdate` blocks only a *simultaneous* add-and-remove of rack IDs, and `Rack.NodeName` is never validated. `metricLabels` had its rule backwards — `=` and `,` are safe inside values because the operator TOML-quotes them; the real constraint is a key charset of `^[A-Za-z0-9_-]+$`. `security: {}` renders nothing. `max-record-size`, `evict-used-pct` and `evict-tenths-pct` are absent from the operator's dynamic allowlist, so editing them cold-restarts. `spec.rollingUpdateBatchSize` is integer-only; percentages are rack-level. Eleven quoted error strings were regenerated from source, and the two TLS surfaces from ACKO#349 — nested `network.tls` and `tls-*` keys inside `network.{service,heartbeat,fabric}` — are now documented.
- Corrected four `aerospike-py` policy defaults against the pinned `aerospike-core 2.0.0` (also #102): `QueryPolicy.max_retries` 2 → 5, `ClientConfig.idle_timeout` 55000 → 30000, `ClientConfig.max_conns_per_node` 300 → 256, and `AdminPolicy.timeout` 1000 → 0 raw / 3000 effective.
- Completed the instrumented-operation list in `aerospike-py-api/reference/observability.md`. #109 corrected `remove` to `delete`; six labels were still missing (`operate_ordered`, `remove_bin`, `batch_apply`, `batch_write_numpy`, `scan`) along with the `*_retry` labels that `batch_write` and `batch_write_numpy` emit on the retry path, so summing the base label undercounts.
- Clarified that `CLIENT_SIDE_RESULT_CODE` is a Rust-internal name, not a constant registered on the Python module — compare `.result_code` against the literal `-1`.
- Added a confirmation guard to `acko-operations` §7, the only irreversible verb in that skill and the only one with no guard (`acko-debugging` and `ackoctl` both have theirs). Deleting a cluster with `cascadeDelete: true` destroys the PVCs; the guard requires stating cluster name, namespace and the **effective** `cascadeDelete` back to the user first. `cascadeDelete` is per-volume (`spec.storage.volumes[].cascadeDelete`, `types_storage.go:94`) defaulted by `spec.storage.{filesystemVolumePolicy,blockVolumePolicy}.cascadeDelete` (`:39,:44,:214`) — there is no top-level `spec.storage.cascadeDelete` to read.
