...
`client.remove()` is labelled `delete` — the label follows the wire operation, not the Python method name (`rust/src/client_ops.rs:98-100`).

**Retries carry their own label.** `batch_write` and `batch_write_numpy` emit a second observation under `batch_write_retry` / `batch_write_numpy_retry` when the retry path runs (`client_ops.rs:455-457`), so summing the base label alone undercounts.
