...
The common trigger is a record carrying a language-specific blob particle type that `aerospike-core` 2.0.0 cannot decode — `PYTHON_BLOB` (8), `JAVA_BLOB` (5) and friends, typically written by an older client against the same namespace. Because it is per-record, catch it inside a scan or batch-read loop to skip the offending record rather than losing the whole request:
