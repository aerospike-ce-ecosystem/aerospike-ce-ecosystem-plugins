...
| `partition_filter_by_range(begin, count)` | `count` partitions from `begin`; `begin + count <= 4096`. `count` of `0` is legal and yields an empty filter. Overflowing 4096 raises `ValueError`. |
