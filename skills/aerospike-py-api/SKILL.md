...
**Import note**: aerospike-py is a Rust/PyO3 extension. All exceptions and constants live directly on the `aerospike_py` module (e.g., `aerospike_py.RecordNotFound`). There is no `aerospike_py.exception` submodule. All return types are NamedTuples — use `record.bins`, `record.meta.gen` attribute access.
