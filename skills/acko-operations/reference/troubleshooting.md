...
| Phase = `ConfigDegraded` | `kubectl get asc <name> -o jsonpath='{.status.pods[*].dynamicConfigChanges}' \| jq` | 2PC dynamic config rollback failed; pods inconsistent; reconcile halted (`ConfigDegradedSkip` events every ~60s) | Do NOT toggle `enableDynamicConfigUpdate`; revert the offending value in spec, then cold-restart pods / reset the phase so reconciliation resumes |
...
| `InProgress` stuck > 5 min | `kubectl get pvc -n <ns> -l app.kubernetes.io/instance=<name>` | PVC Pending, ImagePull failure, scheduling failure | Check StorageClass, image name, resource availability |
...
| Operations stuck `InProgress`, webhook rejects new edit | `kubectl get asc <name> -o jsonpath='{.status.operationStatus.phase}'` | Cannot modify `spec.operations[]` while one is `InProgress` | Wait for the in-flight operation to complete or fail; only then queue the next |
